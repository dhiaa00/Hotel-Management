...
const Room = ({ number, type, inService, cleaned, availableOn }) => {
  return (
    ...
    ">
      ...
      <div className="in-service w-[16.667%]">{inService}</div>
      <div className="cleaned w-[16.667%]">{cleaned}</div>
      <div className="available-on text-center w-[16.667%]">{availableOn}</div>
    </div>
  );
};
